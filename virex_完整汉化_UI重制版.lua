...
BX.module("ui.tabs.main", function(BX)
    ...
    function M.build(tab)
            ...
            callback = function(on)
                if on then
                    ...
                    if okStart == false then
                        win.notify("自动偷蛋", tostring(why), 6)
                        task.spawn(function()
        ...
        auto.onIdle(function(why, whose)
            ...
            task.spawn(function() win.notify("自动偷蛋", text, 7) end)
        end)
        auto.onStop(function(why, whose)
            ...
            if why == "selected egg is gone" then
                task.spawn(function()
                    win.notify("自动偷蛋", "Your egg is gone - stopped. Pick another.", 5)
                end)
            end
            task.spawn(function()
            ...
            if why == "delivered" then
                task.spawn(function()
                    win.notify("自动偷蛋", "Delivered - stopped.", 4)
                end)
            end
        end)
        tab:CreateSection({ name = "透视" })
        tab:CreateToggle({
            name = "蛋透视",
            value = false,
            callback = function(on)
        ...
        autoToggle = tab:CreateToggle({
            ...
            callback = function(on)
                BX.try("farm.autoToggle", function()
                    log.info("toggle -> %s", on and "开启" or "关闭")
                    if on then
                        selfAuto = 0
    ...
    function M.build(tab)
        ...
        fightToggle = tab:CreateToggle({
            ...
            callback = function(v)
                v = v and true or false
                fight.setEnabled(v)
                say("Auto fight " .. (v and "开启" or "关闭"))
            end,
        })
        fightLine = tab:CreateText({ name = "自动战斗", text = "off" })
        tab:CreateButton({
            name = "Claim Mastery",
        ...
        tab:CreateToggle({
            ...
            callback = function(v)
                v = v and true or false
                rift.setAutoTrade(v)
                say("Auto trade-in " .. (v and "开启" or "关闭"))
            end,
        })
        if not M.wired then
...
BX.module("features.treadmill", function(BX)
    ...
    function M.setEnabled(on)
        ...
        log.info("anti treadmill %s", enabled and "开启" or "关闭")
...
BX.module("features.farm.plotcare", function(BX)
    ...
    function M.setPlace(on)
        ...
        log.info("auto place %s", on and "开启" or "关闭")
        return true
    end
    function M.setHatch(on)
        ...
        log.info("auto hatch %s", on and "开启" or "关闭")
        return true
    end
    function M.preview()
        local recs, ES = ownedEggs()
...
BX.module("features.boss", function(BX)
    ...
    function M.setAutoEnter(on)
        ...
        log.info("auto enter %s", autoEnter and "开启" or "关闭")
...
local function req(name)
	refreshBX()
	if not BX or type(BX.require) ~= "function" then return nil, "BX 尚未加载——请先执行主程序" end
	local ok, mod = pcall(function() return BX.require(name) end)
	if not ok then return nil, "模块加载失败：" .. tostring(mod) end
	if mod == nil then return nil, "模块为空：" .. tostring(name) end
	return mod, nil
end
...
local PAGES = { "偷蛋", "农场", "事件", "工具", "设置" }
local pageBtns = {}
local currentPage = "偷蛋"
...
local function pageSteal()
	...
	addToggle(c, "自动偷蛋", false, function(on)
	...
	addButton(c, "刷新目标", T.panel2, function()
		local eggs = req("features.eggs")
		if eggs and type(eggs.list) == "function" then
			local list = eggs.list({}, true)
			setStatus("目标数量：" .. tostring(type(list) == "table" and #list or 0))
		else
			setStatus("目标模块不存在")
		end
	end)
	local c2 = card("透视")
	addToggle(c2, "蛋透视", false, function(on)
		...
		setStatus(ok and ("Egg ESP " .. (on and "开启" or "关闭")) or tostring(e))
	end)
	addToggle(c2, "地块透视", false, function(on)
		...
		setStatus(ok and ("Plot ESP " .. (on and "开启" or "关闭")) or tostring(e))
	end)
	local c3 = card("跑步机")
	addToggle(c3, "防跑步机", false, function(on)
		local tr = req("features.treadmill")
		if tr and type(tr.setEnabled) == "function" then
			tr.setEnabled(on)
			setStatus("Anti Treadmill " .. (on and "开启" or "关闭"))
		else
			setStatus("跑步机模块不存在")
		end
	end)
	...
	addToggle(cHit, "防命中", false, function(on)
		AntiHitEnabled = on and true or false
		setStatus("Anti-Hit " .. (AntiHitEnabled and "开启" or "关闭"))
	...
	for _, row in ipairs(rarities) do
		...
		addToggle(c4, lab, selectedRarities[id] == true, function(on)
			...
			setStatus(ok and ("筛选：" .. tostring(desc)) or tostring(desc))
		end)
	end
	addButton(c4, "清除稀有度筛选", T.panel2, function()
		selectedRarities = {}
		applyFiltersToFarm()
		setStatus("稀有度筛选已清除（全部）")
		showPage("偷蛋")
	end)
	local c5 = card("区域筛选")
	...
	addButton(c5, "清除区域筛选", T.panel2, function()
		selectedAreas = {}
		applyFiltersToFarm()
		setStatus("区域筛选已清除（全部）")
		showPage("偷蛋")
	end)
end
local function pageFarm()
	...
	addToggle(c, "农场自动偷蛋", false, function(on)
		local auto, err = req("features.autosteal")
		if not auto or type(auto.setEnabled) ~= "function" then
		...
		if on and ok == false then
			...
			setStatus(on and "农场循环已开启（持续运行）" or "农场自动偷蛋已关闭")
		end
	end)
	addToggle(c, "保持在跑步机", false, function(on)
		...
		if hold and type(hold.setEnabled) == "function" then
			...
			else setStatus("保持在跑步机 " .. (on and "开启" or "关闭")) end
		else
			setStatus("农场跑步机模块不存在")
		end
	end)
	local c2 = card("地块")
	addToggle(c2, "自动放蛋", false, function(on)
		local pc = req("features.farm.plotcare")
		if pc and type(pc.setPlace) == "function" then
			pc.setPlace(on)
			setStatus("自动放蛋 " .. (on and "开启" or "关闭"))
		else
			setStatus("自动放蛋模块不存在")
		end
	end)
	addToggle(c2, "自动孵蛋", false, function(on)
		local pc = req("features.farm.plotcare")
		if pc and type(pc.setHatch) == "function" then
			pc.setHatch(on)
			setStatus("自动孵蛋 " .. (on and "开启" or "关闭"))
		else
			setStatus("自动孵蛋模块不存在")
		end
	end)
	addButton(c2, "装备最佳宠物", T.ember, function()
...
local function pageCfg()
	clearContent()
	local c = card("主题")
	addNote(c, "VIREX 菜单主题")
	local themes = {
		{ name = "熔岩",  ember = Color3.fromRGB(212, 96, 58),  ember2 = Color3.fromRGB(255, 140, 90),  bg = Color3.fromRGB(10,10,12), panel = Color3.fromRGB(16,16,20), panel2 = Color3.fromRGB(22,22,28) },
		{ name = "青蓝",   ember = Color3.fromRGB(0, 200, 255),  ember2 = Color3.fromRGB(120, 230, 255), bg = Color3.fromRGB(6,10,14),  panel = Color3.fromRGB(12,18,24), panel2 = Color3.fromRGB(18,26,34) },
		{ name = "紫罗兰", ember = Color3.fromRGB(160, 80, 255), ember2 = Color3.fromRGB(200, 140, 255), bg = Color3.fromRGB(10,8,14),  panel = Color3.fromRGB(18,14,28), panel2 = Color3.fromRGB(26,20,40) },
		{ name = "青柠",   ember = Color3.fromRGB(80, 220, 100), ember2 = Color3.fromRGB(140, 255, 160), bg = Color3.fromRGB(8,12,8),   panel = Color3.fromRGB(14,20,14), panel2 = Color3.fromRGB(20,28,20) },
		{ name = "玫红",   ember = Color3.fromRGB(255, 70, 120), ember2 = Color3.fromRGB(255, 140, 180), bg = Color3.fromRGB(12,8,10),  panel = Color3.fromRGB(22,14,18), panel2 = Color3.fromRGB(32,20,26) },
		{ name = "钢铁",  ember = Color3.fromRGB(160, 170, 190),ember2 = Color3.fromRGB(200, 210, 230), bg = Color3.fromRGB(10,10,12), panel = Color3.fromRGB(16,16,20), panel2 = Color3.fromRGB(24,24,28) },
	}
	...
	local c2 = card("速度")
	local speedVal = 50
	addToggle(c2, "启用速度", false, function(on)
		...
		setStatus("速度 " .. (on and ("ON " .. speedVal) or "关闭"))
	end)
	addSlider(c2, "移动速度", 16, 500, 50, function(v)
		speedVal = v
		local spd = req("features.speed")
		if spd and type(spd.setSpeed) == "function" then spd.setSpeed(v) end
	end)
	local c3 = card("配置文件")
	local nameBox = addInput(c3, "配置文件名称")
	addButton(c3, "保存配置", T.ember, function()
		...
		else setStatus("配置文件模块不存在") end
	end)
	addButton(c3, "加载配置", T.panel2, function()
		local prof = req("core.profiles")
	...
	addButton(c3, "删除配置", T.danger, function()
		local prof = req("core.profiles")
...
local function pageTools()
	clearContent()
	local c = card("性能")
	addToggle(c, "FPS 优化模式", false, function(on)
		local fps = req("features.fps")
		if fps and type(fps.setEnabled) == "function" then
			fps.setEnabled(on)
			setStatus("FPS Mode " .. (on and "开启" or "关闭"))
		else
			setStatus("FPS 模块不存在")
		end
	end)
	local c2 = card("服务器")
	addButton(c2, "加入人数最少服务器", T.ember, function()
		local srv = req("features.misc.servers")
		if srv and type(srv.lowestServer) == "function" then
			srv.lowestServer()
			setStatus("已请求加入人数最少服务器")
		else
			setStatus("服务器模块不存在")
		end
	end)
	addButton(c2, "切换服务器", T.panel2, function()
		local srv = req("features.misc.servers")
	...
	local c3 = card("Webhook 通知")
	local urlBox = addInput(c3, "Webhook 地址")
	addToggle(c3, "启用 Webhook", false, function(on)
		...
		if hook and type(hook.setEnabled) == "function" then
			...
			hook.setEnabled(on)
			setStatus("Webhook " .. (on and "开启" or "关闭"))
		else
			setStatus("Webhook 模块不存在")
		end
	end)
	addButton(c3, "发送测试", T.panel2, function()
		local hook = req("features.misc.webhook")
		if hook then
			...
			setStatus("Webhook 测试已发送")
		else
			setStatus("Webhook 模块不存在")
		end
	end)
	local c4 = card("Discord")
	addButton(c4, "复制 VIREX Discord", T.ember, function()
...
local function pageSet()
	clearContent()
	local c = card("配置文件")
	local nameBox = addInput(c, "配置文件名称")
	addButton(c, "保存配置", T.ember, function()
		local prof = req("core.profiles")
		if prof and type(prof.save) == "function" then
			local ok, msg = prof.save(nameBox.Text)
			setStatus(tostring(msg or ok))
	...
	addButton(c, "加载配置", T.panel2, function()
		local prof = req("core.profiles")
		if prof and type(prof.load) == "function" then
			local ok, msg = prof.load(nameBox.Text)
			setStatus(tostring(msg or ok))
	...
	addButton(c, "删除配置", T.danger, function()
		local prof = req("core.profiles")
		if prof and type(prof.delete) == "function" then
			local ok, msg = prof.delete(nameBox.Text)
			setStatus(tostring(msg or ok))
	...
	addNote(c, "Uses core.profiles from hub backend.")
end
local builders = {
	偷蛋 = pageSteal,
	农场 = pageFarm,
...
if linked then
	setStatus("已连接 · " .. tostring(info) .. " · drag top · X closes")
else
	setStatus("未检测到 BX——请先执行主程序，再执行此界面")
end
